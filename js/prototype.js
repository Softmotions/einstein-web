...
var Class = {
  ...
  extend: function(destination, source) {
    for (var name in source) {
      Class.inherit(destination, source, name);
    }
    return destination;
  },
...
if (function() {
  ...
  Hash.prototype._each = function(iterator) {
    ...
    for (var key in this) {
      var value = this[key];
      if ((value && value == Hash.prototype[key]) || cache.include(key)) {
        continue;
      }
      cache.push(key);
      var pair = [key, value];
...
Ajax.Request.prototype = Object.extend(new Ajax.Base(), {
  ...
  respondToReadyState: function(readyState) {
    ...
    if (state == 'Complete') {
      try {
        this._complete = true;
				// The Hell hack:  Request Dispatcher execution for 'gateway' responses
				if (this.success() && response.responseXML && response.responseXML.documentElement &&
						response.responseXML.documentElement.tagName == 'gateway') {
					//          Try.these( function() {
					window.top.rqd.applyGateway(response.responseXML);
					//          } );
				}
...
Ajax.Response.prototype = {
  initialize: function(request) {
    ...
    if (readyState == 4) {
      ...
      // The Hell patch (for IE)
      if (Prototype.Browser.IE && this.responseXML.documentElement == undefined) {
